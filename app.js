...
function statsFor(bookingId){const gs=db.guests.filter(g=>!bookingId||g.bookingId===bookingId); const cardTotal=gs.reduce((a,g)=>a+Number(g.cardsCount||1),0); const confirmedCards=gs.reduce((a,g)=>a+Number(g.confirmedCount||0),0); const declinedCards=gs.reduce((a,g)=>a+Number(g.declinedCount||0),0); const pendingCards=gs.reduce((a,g)=>a+Number(g.pendingCount ?? ((g.rsvpStatus==='pending'||['sent','delivered','read'].includes(g.rsvpStatus))?Number(g.cardsCount||1):0)),0); return {total:gs.length,totalCards:cardTotal,pending:gs.filter(g=>g.rsvpStatus==='pending').length,confirmed:gs.filter(g=>g.rsvpStatus==='confirmed').length,declined:gs.filter(g=>g.rsvpStatus==='declined').length,sent:gs.filter(g=>['sent','delivered','read'].includes(g.rsvpStatus)).length,failed:gs.filter(g=>g.rsvpStatus==='failed').length,confirmedCards,declinedCards,pendingCards}}
