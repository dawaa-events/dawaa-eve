...
// DAWAA Urgent Guest Sync
let urgentSyncInFlight = false;
function normalizeUrgentPhone(v=''){const d=String(v||'').replace(/\D/g,''); return d.length===8?'968'+d:d;}
function normalizeRemoteGuest(g, fallbackBookingId=''){
 const cards=Number(g.cardsCount ?? g.cards_count ?? g.cards ?? 1)||1;
 return {
  id:String(g.localId||g.local_id||(g.id?('remote_'+g.id):uid())),
  dbGuestId:g.dbGuestId||g.id||null,
  bookingId:g.bookingId||g.booking_id||fallbackBookingId||getSelectedBookingId()||db.bookings[0]?.id||'',
  guestName:g.guestName||g.guest_name||g.name||'-',
  phoneNumber:normalizeUrgentPhone(g.phoneNumber||g.phone_number||g.phone||''),
  cardsCount:cards,
  rsvpStatus:g.rsvpStatus||g.rsvp_status||g.status||'pending',
  confirmedCount:Number(g.confirmedCount??g.confirmed_count??0),
  declinedCount:Number(g.declinedCount??g.declined_count??0),
  pendingCount:Number(g.pendingCount??g.pending_count??cards),
  invitationSentAt:g.invitationSentAt||g.invitation_sent_at||null,
  deliveredAt:g.deliveredAt||g.delivered_at||null,
...
async function deleteGuestFromServer(guest){
  ...
  try{
    ...
    const url='/api/guests-sync'+(dbId?('?id='+encodeURIComponent(dbId)):'');
    ...
    console.warn('[deleteGuestFromServer:force]',e);
    return false;
  }
}


function guestIdentityKey(g){
  return [
...
function getDeletedGuestKeys(){
  try{return JSON.parse(localStorage.getItem('dawaa_deleted_guest_keys')||'[]')}catch(e){return[]}
}
function rememberDeletedGuestKey(g){
  const key=guestIdentityKey(g);
  if(!key || key.includes('||')) return;
  const list=[...new Set([...getDeletedGuestKeys(), key])].slice(-2000);
  localStorage.setItem('dawaa_deleted_guest_keys', JSON.stringify(list));
}
function isDeletedGuestKey(g){
  return getDeletedGuestKeys().includes(guestIdentityKey(g));
}
function forgetDeletedGuestKey(g){
  const key=guestIdentityKey(g);
  const list=getDeletedGuestKeys().filter(x=>x!==key);
  localStorage.setItem('dawaa_deleted_guest_keys', JSON.stringify(list));
}


/* SAFE UI/SYNC PATCH - does not touch WhatsApp sending files */
...
function guestOrderForDedupe(g){
  return Number(g?.startOrder || g?.start_order || g?.orderNumber || g?.order_number || 0) || 0;
}
function guestDedupeKey(g){
  const booking=String(g?.bookingId || getSelectedBookingId() || '');
  const phone=String(g?.phoneNumber || '').replace(/\D/g,'');
  const name=normalizeGuestNameForDedupe(g?.guestName || '');
  const order=guestOrderForDedupe(g);
  const cards=Number(g?.cardsCount || 1) || 1;

  // أهم حل للتكرار: إذا فيه رقم ترتيب، فهو هوية الضيف داخل المناسبة
  // حتى لو نسخة ثانية جايه من الفرز الأبجدي.
  if(booking && order) return `booking-order|${booking}|${order}`;

  // إذا ما فيه ترتيب، استخدم الرقم + الاسم.
  if(booking && phone && name) return `booking-phone-name|${booking}|${phone}|${name}`;

  // إذا النسخة الأبجدية ناقصها رقم/ترتيب، استخدم الاسم + عدد البطاقات داخل نفس المناسبة.
  if(booking && name) return `booking-name-cards|${booking}|${name}|${cards}`;

  return `fallback|${booking}|${phone}|${name}|${cards}`;
}
function guestDedupeScore(g){
  return (
    (guestOrderForDedupe(g)?200:0)+
    (g?.dbGuestId?120:0)+
    (g?.phoneNumber?100:0)+
    (g?.metaMessageId?80:0)+
    (['confirmed','declined','sent','delivered','read'].includes(g?.rsvpStatus)?50:0)+
    Number(g?.confirmedCount||0)
  );
}
function mergeGuestRecords(a={}, b={}){
  const preferred = guestDedupeScore(b)>guestDedupeScore(a) ? b : a;
  const other = preferred===b ? a : b;
  return {
    ...other,
    ...preferred,
    // لا نخسر المعلومات المهمة من النسخة الثانية
    phoneNumber: preferred.phoneNumber || other.phoneNumber || '',
    dbGuestId: preferred.dbGuestId || other.dbGuestId || null,
    metaMessageId: preferred.metaMessageId || other.metaMessageId || null,
    invitationSentAt: preferred.invitationSentAt || other.invitationSentAt || null,
    deliveredAt: preferred.deliveredAt || other.deliveredAt || null,
    readAt: preferred.readAt || other.readAt || null,
    repliedAt: preferred.repliedAt || other.repliedAt || null,
    orderNumber: preferred.orderNumber || other.orderNumber || null,
    startOrder: preferred.startOrder || other.startOrder || preferred.orderNumber || other.orderNumber || null,
    entryCardUrl: preferred.entryCardUrl || other.entryCardUrl || '',
    entryCardUrls: (preferred.entryCardUrls?.length ? preferred.entryCardUrls : other.entryCardUrls) || [],
    confirmedCount: Math.max(Number(preferred.confirmedCount||0), Number(other.confirmedCount||0)),
    declinedCount: Math.max(Number(preferred.declinedCount||0), Number(other.declinedCount||0)),
    pendingCount: Number(preferred.pendingCount ?? other.pendingCount ?? preferred.cardsCount ?? other.cardsCount ?? 1),
  };
}
function dedupeGuestArray(list=[]){
  const map=new Map();

  // تمريرة أولى بالمفتاح الأقوى
  for(const g of list||[]){
    const key=guestDedupeKey(g);
    const old=map.get(key);
    map.set(key, old ? mergeGuestRecords(old,g) : g);
  }

  // تمريرة ثانية: دمج النسخ التي لها نفس الاسم داخل نفس المناسبة
  // خصوصًا نسخة رقم الترتيب ونسخة الأبجدية.
  const byName=new Map();
  for(const g of map.values()){
    const booking=String(g?.bookingId || getSelectedBookingId() || '');
    const name=normalizeGuestNameForDedupe(g?.guestName || '');
    if(!booking || !name){
      byName.set(`unique|${Math.random()}`, g);
      continue;
    }
    const key=`booking-name|${booking}|${name}`;
    const old=byName.get(key);
    byName.set(key, old ? mergeGuestRecords(old,g) : g);
  }

  return [...byName.values()].sort((a,b)=>{
    const ao=guestOrderForDedupe(a), bo=guestOrderForDedupe(b);
    if(ao && bo) return ao-bo;
    if(ao) return -1;
    if(bo) return 1;
    return String(a.guestName||'').localeCompare(String(b.guestName||''),'ar');
  });
}
...
async function repairGuestDuplicatesAndSync(){
  ...
  try{
    ...
    if(data.guests && typeof mergeGuestsFromServer==='function') mergeGuestsFromServer(data.guests);
    db.guests=dedupeGuestArray(db.guests||[]);
    localStorage.setItem('dawaa_guests',JSON.stringify(db.guests||[]));
    render();
    ...
    showToast('تم تنظيف العرض محليًا، وتعذر تنظيف السيرفر');
  }
}
async function deleteAllGuestsForCurrentBooking(){
  ...
  try{
    ...
    if(typeof loadGuestsFromServer==='function') await loadGuestsFromServer({silent:true,force:true});
    render();
  }catch(e){
    console.warn('[deleteAllGuestsForCurrentBooking]',e);
    showToast('تم الحذف محلياً، وتعذر حذف نسخة المزامنة');
  }
}
...
async function loadGuestsFromServer({silent=true,force=false}={}){
 if(urgentSyncInFlight) return false;
 urgentSyncInFlight=true;
 try{
  ...
  const changed=mergeGuestsFromServer(data.guests||[]); db.guests=dedupeGuestArray(db.guests||[]); db.guests=dedupeGuestArray(db.guests||[]);
...
function arabicDayLabel(n){
  if(n === null || n === undefined) return 'غير محدد';
  if(n < 0) return `انتهت منذ ${Math.abs(n)} ${Math.abs(n)===1?'يوم':'أيام'}`;
  if(n === 0) return 'اليوم';
  if(n === 1) return 'غداً';
  if(n === 2) return 'بعد يومين';
  return `بعد ${n} أيام`;
}
function countdownInfo(b){
  const remaining = daysUntil(b?.eventDate);
  const guests = guestsForBookingDeduped(b?.id);
  const hasSent = guests.some(g=>g.invitationSentAt || ['sent','delivered','read','confirmed','declined'].includes(g.rsvpStatus));
  const allConfirmedOrDeclined = guests.length > 0 && guests.every(g=>['confirmed','declined','checked-in'].includes(g.rsvpStatus));
  const cardsReady = Boolean(b?.cardsReady || b?.cards_ready);
  let nextTitle = 'إرسال الدعوات';
  let nextText = 'لم يبدأ الإرسال بعد';
  let cls = 'send';
  if(hasSent && !allConfirmedOrDeclined){
    nextTitle = 'متابعة الردود';
    nextText = 'بانتظار ردود بعض الضيوف';
    cls = 'wait';
  }
  if(allConfirmedOrDeclined && !cardsReady){
    nextTitle = 'إصدار البطاقات';
    nextText = 'الردود اكتملت تقريباً';
    cls = 'cards';
  }
  if(cardsReady){
    nextTitle = 'إرسال البطاقات';
    nextText = 'البطاقات جاهزة للتسليم';
    cls = 'ready';
  }
  if(remaining !== null && remaining <= 2 && !cardsReady){
    nextTitle = 'تنبيه عاجل';
    nextText = 'تبقى وقت قصير على المناسبة';
    cls = 'urgent';
  }
  return {remaining, eventLabel:arabicDayLabel(remaining), nextTitle, nextText, cls};
}
function countdownWidget(b, compact=false){
  const c = countdownInfo(b);
  const target = b?.eventDate ? new Date(b.eventDate).getTime() : 0;
  return `<div class="event-countdown ${compact?'compact':''} ${c.cls}" data-countdown-target="${target}">
    <div class="count-main">
      <span>العد التنازلي للمناسبة</span>
      <b>${escapeHtml(c.eventLabel)}</b>
      <small class="live-countdown">جارٍ حساب الوقت المتبقي...</small>
    </div>
    <div class="count-next">
      <span>الخطوة القادمة</span>
      <b>${escapeHtml(c.nextTitle)}</b>
      <small>${escapeHtml(c.nextText)}</small>
    </div>
  </div>`;
}
function formatLiveCountdown(ms){
  if(!ms || ms < 0) return 'انتهت المناسبة';
...
function updateLiveCountdowns(){
  document.querySelectorAll('[data-countdown-target]').forEach(el=>{
    const target=Number(el.getAttribute('data-countdown-target')||0);
    const small=el.querySelector('.live-countdown');
    if(small && target) small.textContent=formatLiveCountdown(target-Date.now());
  });
}
...
function timelineTimeLabel(key, value){
...
function saveBookingSettings(bookingId, settings){
  const all=JSON.parse(localStorage.getItem('dawaa_booking_settings')||'{}');
  all[bookingId]={...getBookingSettings(bookingId), ...settings};
  localStorage.setItem('dawaa_booking_settings', JSON.stringify(all));
}
function usedClientCards(bookingId){
  return guestsForBookingDeduped(bookingId).filter(g=>g.source==='client').reduce((a,g)=>a+Number(g.cardsCount||1),0);
}
...
function permissionsPanel(b){
 ...
 return `<section class="panel permissions-panel">
  <div class="section-title-row"><div><h2>صلاحيات العميل</h2><p class="muted">تتحكم الإدارة بإضافة وتعديل الضيوف وحد البطاقات للعميل.</p></div></div>
  <div class="permission-grid">
    <label><input type="checkbox" ${st.allowClientAdd?'checked':''} onchange="saveBookingSettings('${b.id}',{allowClientAdd:this.checked});showToast('تم تحديث الصلاحيات')"> يسمح بإضافة ضيوف</label>
    <label><input type="checkbox" ${st.allowClientEdit?'checked':''} onchange="saveBookingSettings('${b.id}',{allowClientEdit:this.checked});showToast('تم تحديث الصلاحيات')"> يسمح بالتعديل</label>
    <label><input type="checkbox" ${st.allowClientDelete?'checked':''} onchange="saveBookingSettings('${b.id}',{allowClientDelete:this.checked});showToast('تم تحديث الصلاحيات')"> يسمح بالحذف</label>
    <label><input type="checkbox" ${st.allowBackupList?'checked':''} onchange="saveBookingSettings('${b.id}',{allowBackupList:this.checked});showToast('تم تحديث الصلاحيات')"> يسمح بقائمة الاحتياط</label>
  </div>
  <div class="field"><label>حد العميل حسب عدد البطاقات</label><input type="number" value="${Number(st.clientCardLimit||0)}" onchange="saveBookingSettings('${b.id}',{clientCardLimit:Number(this.value||0)});showToast('تم حفظ الحد')"></div>
  <small>المستخدم حالياً: ${usedClientCards(b.id)} / ${Number(st.clientCardLimit||0) || 'غير محدود'} بطاقة.</small>
 </section>`;
}
...
function adminGuestDetails(id){
 ...
 if(!g) return adminGuests();
 const b=db.bookings.find(x=>x.id===g.bookingId);
 adminShell(`<div class="guest-detail-page">
  <div class="calm-hero-card">
    <div>
      <span class="eyebrow">ملف الضيف</span>
      <h1>${escapeHtml(g.guestName)}</h1>
      <p>${escapeHtml(g.phoneNumber)} • ${Number(g.cardsCount||1)} بطاقات • ${escapeHtml(b?.eventName||'')}</p>
    </div>
    <div class="calm-hero-actions">
      <button class="btn btn-primary" onclick="sendOne('${g.id}')">إرسال</button>
      <button class="btn btn-secondary" onclick="editGuest('${g.id}')">تعديل</button>
      <button class="btn btn-ghost" onclick="go('/admin/guests')">رجوع</button>
    </div>
  </div>
  ...
  <section class="panel"><h2>Timeline الإرسال</h2><div class="proof-timeline horizontal-proof">
    <div class="tl"><span class="dot"></span> تم إنشاء سجل الضيف <small>${timelineTimeLabel('created',g.createdAt||g.updatedAt)}</small></div>
...
function normalizeRemoteGuest(g, fallbackBookingId=''){
  ...
  return {
    ...
    confirmedCount: Number(g.confirmedCount ?? g.confirmed_count ?? 0),
    declinedCount: Number(g.declinedCount ?? g.declined_count ?? 0),
    pendingCount: Number(g.pendingCount ?? g.pending_count ?? cardsCount),
    invitationSentAt: g.invitationSentAt || g.invitation_sent_at || null,
    deliveredAt: g.deliveredAt || g.delivered_at || null,
    readAt: g.readAt || g.read_at || null,
    repliedAt: g.repliedAt || g.replied_at || null,
...
function emergencySecurityCheck(){
  const user = JSON.parse(localStorage.getItem('dawaa_user')||'null');
  if(!user && location.hash.includes('/admin')) { location.hash='/login'; return false; }
  return true;
}

function render(){ if(!emergencySecurityCheck()) return; 
...
const stories=[
 ...
 ['لوحة العميل الحية', 'إحصائيات واضحة: حاضر، معتذر، لم تؤكد البطاقات، ورسائل.', 'layout-dashboard'],
...
function renderLogin(){app.innerHTML=`<main class="login-v2">
  <section class="login-v2-panel">
    <div class="login-brand">
      <img src="assets/dawaa-logo-purple.png" alt="دعوة">
      <span>دعوة</span>
    </div>
    ...
    <div class="login-mini-grid">
      <div><b>WhatsApp</b><small>إرسال ومتابعة</small></div>
      <div><b>QR</b><small>بطاقات دخول</small></div>
      <div><b>LIVE</b><small>بوابة العميل</small></div>
    </div>
  </section>
  <section class="login-v2-card">
...
function guestsForBookingDeduped(bookingId=''){
  const list=(db.guests||[]).filter(g=>!bookingId || g.bookingId===bookingId);
  return typeof dedupeGuestArray==='function' ? dedupeGuestArray(list) : list;
}

function statsFor(bookingId){
 const gs=guestsForBookingDeduped(bookingId);
 const totalCards=gs.reduce((a,g)=>a+Number(g.cardsCount||1),0);
 const confirmedCards=gs.reduce((a,g)=>a+Number(g.confirmedCount||0),0);
 const declinedCards=gs.reduce((a,g)=>a+Number(g.declinedCount||0),0);
 const pendingCards=Math.max(0,totalCards-confirmedCards-declinedCards);
 return {
  total:totalCards,
  totalPeople:gs.length,
  totalCards,
  pending:pendingCards,
  confirmed:confirmedCards,
  declined:declinedCards,
  sent:gs.filter(g=>['sent','delivered','read'].includes(g.rsvpStatus)).reduce((a,g)=>a+Number(g.pendingCount||g.cardsCount||1),0),
  failed:gs.filter(g=>g.rsvpStatus==='failed').reduce((a,g)=>a+Number(g.cardsCount||1),0),
  confirmedCards,
  declinedCards,
  pendingCards
 };
...
function filteredGuestsList(){
 ...
 const bookingId=getSelectedBookingId();
 return sortGuestsByOrder(guestsForBookingDeduped(bookingId).filter(g=>
...
async function applyEntryCardUrlsToGuests(){
  ...
  let guests=(db.guests||[]).filter(g=>g.bookingId===bookingId);

  // إذا ما فيه ترتيب محفوظ، احسبه تلقائياً من عدد البطاقات.
  if(!guests.some(g=>Number(g.startOrder||g.orderNumber||0))){
    guests=calculateOrderMetaForGuests(guests,bookingId);
  }
  ...
  for(const g of guests){
    ...
    Object.assign(g,{...meta,orderNumber:start,startOrder:start});
    updated.push({...g,...meta,orderNumber:start,startOrder:start});
  }

  if(!updated.length) return showToast('لا يوجد ضيوف لديهم رقم ترتيب');

  db.guests=(db.guests||[]).map(g=>{
    const u=updated.find(x=>x.id===g.id || x.dbGuestId===g.dbGuestId);
    return u||g;
  });
...
function sortGuestsForOrderRepair(list=[]){
  return [...list].sort((a,b)=>{
    const ao=Number(a.orderNumber||a.startOrder||0);
    const bo=Number(b.orderNumber||b.startOrder||0);
    if(ao && bo) return ao-bo;
    if(ao) return -1;
    if(bo) return 1;
...
function calculateOrderMetaForGuests(guests=[], bookingId=getSelectedBookingId()){
  let cursor=1;
  const ordered=sortGuestsForOrderRepair(guests);
  return ordered.map(g=>{
    const currentStart=Number(g.startOrder||g.orderNumber||0) || cursor;
    const cards=Number(g.cardsCount||1)||1;
    const meta=packEntryCardMeta(currentStart,cards,bookingId);
    cursor=currentStart+cards;
    return {...g,...meta,orderNumber:currentStart,startOrder:currentStart};
  });
}
async function repairOrderNumbersForCurrentBooking(){
  const bookingId=getSelectedBookingId();
  if(!bookingId) return showToast('اختاري مناسبة أولاً');
  const guests=(db.guests||[]).filter(g=>g.bookingId===bookingId);
  if(!guests.length) return showToast('لا يوجد ضيوف لإصلاح الترتيب');
  const repaired=calculateOrderMetaForGuests(guests,bookingId);
  db.guests=(db.guests||[]).map(g=>{
    const r=repaired.find(x=>x.id===g.id || x.dbGuestId===g.dbGuestId);
    return r||g;
  });
  render();
  await saveGuestsToServer(repaired,{silent:true});
  await loadGuestsFromServer({silent:true,force:true});
  render();
  showToast(`تم إصلاح وحفظ ترتيب ${repaired.length} ضيف`);
}
...
function cardStatusSummary(g){
 ...
 const confirmed=Math.max(0, Number(g.confirmedCount||0));
 const declined=Math.max(0, Number(g.declinedCount||0));
 const pending=Math.max(0, Number(g.pendingCount ?? Math.max(0,total-confirmed-declined)));
 if(g.rsvpStatus==='confirmed' || confirmed>0){
  return {label:`تم تأكيد ${confirmed} من أصل ${total}`, sub:`حاضر ${confirmed} • معتذر ${declined} • متبقي ${pending}`, cls:'confirmed'};
 }
 if(g.rsvpStatus==='declined'){
  return {label:`تم الاعتذار عن ${declined||total} من أصل ${total}`, sub:`حاضر ${confirmed} • معتذر ${declined||total}`, cls:'declined'};
 }
 if(['sent','delivered','read'].includes(g.rsvpStatus)){
  return {label:`بانتظار الرد على ${total} ${total===1?'بطاقة':'بطاقات'}`, sub:`تم الإرسال ولم يكتمل التأكيد`, cls:'sent'};
 }
 if(g.rsvpStatus==='failed'){
  return {label:`فشل الإرسال`, sub:`راجعي الرقم أو أعيدي الإرسال`, cls:'failed'};
 }
 return {label:`لم يتم التأكيد بعد`, sub:`${total} ${total===1?'بطاقة مخصصة':'بطاقات مخصصة'}`, cls:'pending'};
}
...
function decodeArabicFileBuffer(buffer){
  const bytes = new Uint8Array(buffer);
  const hasUtf8Bom = bytes.length >= 3 && bytes[0] === 0xEF && bytes[1] === 0xBB && bytes[2] === 0xBF;
  const decoders = hasUtf8Bom ? ['utf-8'] : ['utf-8', 'windows-1256', 'iso-8859-6'];
  for (const enc of decoders) {
    try {
      const text = new TextDecoder(enc, { fatal:false }).decode(bytes);
      if (/[\u0600-\u06FF]/.test(text) && !text.includes('�')) return text.replace(/^\ufeff/, '');
      if (enc === 'utf-8' && !text.includes('�')) return text.replace(/^\ufeff/, '');
    } catch(e){}
  }
  return new TextDecoder('utf-8', { fatal:false }).decode(bytes).replace(/^\ufeff/, '');
}
function parseCsvLineArabic(line){
...
function csvEscapeArabic(value){
  const s=String(value ?? '').replaceAll('"','""');
  return `"${s}"`;
}
function downloadArabicCsv(filename, rows){
  const csv='\ufeff'+rows.map(r=>r.map(csvEscapeArabic).join(',')).join('\r\n');
...
function entryCardMatchStats(bookingId=getSelectedBookingId()){
  const guests=guestsForBookingDeduped(bookingId);
...
function autoMatchEntryCards(show=true){
  ...
  let guests=guestsForBookingDeduped(bookingId).map(g=>({...g, entryCardFile:null, entryCardFileId:null, entryCardMatchedAt:null}));
  const used=new Set();
...
function getTemplateVars(b){
 return {
  ...
  host_one:b.hostOne||'',
  host_two:b.hostTwo||'',
  bride_name:b.brideName||'',
  groom_name:b.groomName||'',
  cards_count:'عدد البطاقات',
  invitation_image_url:b.invitationImageUrl||''
 };
}
...
function saveMetaTemplateVars(bookingId){
 const b=db.bookings.find(x=>x.id===bookingId)||db.bookings.find(x=>x.id===getSelectedBookingId());
 if(!b) return showToast('اختاري مناسبة أولاً');
 b.hostOne=$('#tplHostOne')?.value||'';
 b.hostTwo=$('#tplHostTwo')?.value||'';
...
function adminSend(){
 const selectedBooking=getSelectedBookingId();
 const booking=db.bookings.find(b=>b.id===selectedBooking);
 const guests=filteredGuestsList(); const pending=guestsForBookingDeduped(selectedBooking).filter(g=>g.rsvpStatus==='pending');
...
async function apiSendGuests(guestList, label='الدعوات'){
 ...
 try{
  ...
  const data=await res.json();
  if(!res.ok || data.success===false) throw new Error(data.firstError || data.message || data.error || 'فشل الإرسال');
  const sentIds=new Set((data.results||[]).filter(r=>r.status==='sent').map(r=>r.guestId));
  const failedIds=new Set((data.results||[]).filter(r=>r.status!=='sent').map(r=>r.guestId));
  db.guests=db.guests.map(g=>sentIds.has(g.id)?{...g,rsvpStatus:'sent',invitationSentAt:now(),metaMessageId:(data.results.find(r=>r.guestId===g.id)||{}).messageId||g.metaMessageId}:failedIds.has(g.id)?{...g,rsvpStatus:'failed'}:g);
...
async function sendInvitations(){
 const bookingId=getSelectedBookingId();
 const guests=guestsForBookingDeduped(bookingId).filter(g=>['pending','failed','sent'].includes(g.rsvpStatus));
...
function adminWorkspace(){
 ...
 const b=db.bookings.find(x=>x.id===id) || db.bookings[0];
 if(!b) return adminOperations();
 const s=statsFor(b.id);
 const guests=guestsForBookingDeduped(b.id);
 const recent=guests.slice(0,4);
 adminShell(`<div class="workspace-page-v2">
  <div class="workspace-hero-v2">
    <div>
      <span class="eyebrow">مساحة عمل المناسبة</span>
      <h1>${escapeHtml(b.eventName)}</h1>
      <p>${fmt(b.eventDate)} • ${escapeHtml(b.venueName||'بدون قاعة')} • ${escapeHtml(b.receptionTime||'')}</p>
    </div>
    <div class="workspace-hero-actions">
      <button class="btn btn-primary" onclick="go('/admin/send')">صفحة الإرسال</button>
      <button class="btn btn-secondary" onclick="go('/admin/guests')">إدارة الضيوف</button>
    </div>
  </div>

  ${countdownWidget(b)}

  <div class="workspace-timeline-v2">
    <span class="done">الحجز</span>
    <span class="${s.total?'done':'wait'}">الضيوف</span>
    <span class="${s.sent?'done':'wait'}">الدعوات</span>
    <span class="${s.confirmed||s.declined?'done':'active'}">الردود</span>
    <span class="${b.cardsReady?'done':'wait'}">البطاقات QR</span>
    <span class="${b.screenUploaded?'done':'wait'}">الشاشة</span>
  </div>
...
function clientGuestDetails(id){
 ...
 const linkedId=currentUser?.bookingId||account?.bookingId||getSelectedBookingId()||db.bookings[0]?.id;
 ...
 const steps=[
  {key:'created', title:'إنشاء الضيف', time:g.createdAt||g.updatedAt, done:Boolean(g.createdAt||g.updatedAt)},
  {key:'sent', title:'إرسال الدعوة', time:g.invitationSentAt, done:Boolean(g.invitationSentAt)},
  {key:'delivered', title:'تسليم الرسالة', time:g.deliveredAt, done:Boolean(g.deliveredAt)},
  {key:'read', title:'قراءة الرسالة', time:g.readAt, done:Boolean(g.readAt)},
  {key:'reply', title:'تسجيل الرد', time:g.repliedAt, done:Boolean(g.repliedAt)}
 ];
 ...
 div.innerHTML=`<div class="client-detail-card"><button class="client-detail-close" onclick="this.closest('.client-detail-overlay').remove()">إغلاق</button>
  ...
  <h3>Timeline الإرسال الحقيقي</h3><div class="client-proof-timeline">${steps.map(s=>`<div class="${timelineStepClass(s.key,s.time,s.done)}"><span></span><b>${s.title}</b><small>${timelineTimeLabel(s.key,s.time)}</small></div>`).join('')}</div>
 </div>`;
...
function renderClient(route){
 ...
 const confirmed=guests.reduce((a,g)=>a+Number(g.confirmedCount||0),0);
 const declined=guests.reduce((a,g)=>a+Number(g.declinedCount||0),0);
 const pending=Math.max(0,total-confirmed-declined);
 const sent=guests.filter(g=>g.invitationSentAt||['sent','delivered','read','confirmed','declined'].includes(g.rsvpStatus)).reduce((a,g)=>a+Number(g.cardsCount||1),0);
 const interaction=total?Math.round(((confirmed+declined)/total)*100):0;
 const sentPct=total?Math.round((sent/total)*100):0;
 const rows=guests.length?guests.map(g=>{const cards=Number(g.cardsCount||1),c=Number(g.confirmedCount||0),d=Number(g.declinedCount||0),p=Math.max(0,cards-c-d);return `<tr onclick="clientGuestDetails('${g.id}')"><td><b>${escapeHtml(g.guestName)}</b><small>${escapeHtml(g.phoneNumber||'')}</small></td><td>${cards}</td><td><span class="pill green">${c}</span></td><td><span class="pill red">${d}</span></td><td><span class="pill orange">${p}</span></td><td>${statusBadge(g.rsvpStatus)}</td><td><small>${timeLabel(g.repliedAt||g.readAt||g.deliveredAt||g.invitationSentAt)}</small></td></tr>`}).join(''):`<tr><td colspan="7"><div class="empty-state"><b>لا يوجد ضيوف بعد</b><span>ستظهر قائمة الضيوف بعد رفعها من الإدارة.</span></div></td></tr>`;
 app.innerHTML=`<div class="client-live-page client-urgent-page route-page">
  <div class="client-live-top"><div><span class="badge b-green">LIVE</span><h1>متابعة ${escapeHtml(b.eventName)}</h1><p>آخر تحديث: ${currentUpdateTime()} • بيانات هذه المناسبة فقط</p></div><button class="btn btn-secondary logout-clear" onclick="logout()">تسجيل الخروج</button></div>
  <section class="client-event-hero"><div><span class="eyebrow">بيانات المناسبة</span><h2>${escapeHtml(b.eventName)}</h2><p>${fmt(b.eventDate)} • ${escapeHtml(b.venueName||'لم يتم تحديد القاعة')} • ${escapeHtml(b.receptionTime||'')}</p></div><div>${countdownWidget(b,true)}</div></section>
  <section class="client-detail-grid"><div class="client-stat green"><span>حاضر</span><b>${confirmed}</b><small>بطاقة مؤكدة</small></div><div class="client-stat orange"><span>لم يؤكد</span><b>${pending}</b><small>بطاقة بانتظار الرد</small></div><div class="client-stat red"><span>معتذر</span><b>${declined}</b><small>بطاقة معتذرة</small></div><div class="client-stat blue"><span>الإرسال</span><b>${sent}/${total}</b><small>${sentPct}%</small></div></section>
  <section class="client-progress-grid"><div class="client-progress purple"><h3>نسبة التفاعل</h3><b>${interaction}%</b><div class="progress"><span style="width:${interaction}%"></span></div><p>الحضور والاعتذارات من إجمالي البطاقات.</p></div><div class="client-progress blue"><h3>نسبة الإرسال</h3><b>${sentPct}%</b><div class="progress"><span style="width:${sentPct}%"></span></div><p>عدد البطاقات التي بدأ إرسالها.</p></div></section>
  <section class="panel"><div class="section-title-row"><div><h2>تفاصيل الضيوف</h2><p class="muted">اضغط على اسم الضيف لعرض Timeline الإرسال بالدقيقة.</p></div></div><div class="client-table-wrap"><table class="client-details-table"><thead><tr><th>الضيف</th><th>البطاقات</th><th>حاضر</th><th>معتذر</th><th>لم يؤكد</th><th>الحالة</th><th>آخر تحديث</th></tr></thead><tbody>${rows}</tbody></table></div></section>
 </div>`;
 afterRender();
}
...
function openClientGuestModal(){openGuestModal(); setTimeout(()=>{
 if($('#gBooking')) $('#gBooking').value=(currentUser?.bookingId||getSelectedBookingId());
 if($('#gCategory')) $('#gCategory').value='edited';
},30)}
function openClientBackupModal(){openGuestModal(); setTimeout(()=>{
 if($('#gBooking')) $('#gBooking').value=(currentUser?.bookingId||getSelectedBookingId());
 if($('#gCategory')) $('#gCategory').value='backup';
},30)}
...
function cmdSearch(){const q=$('#cmdInput').value||''; const items=[['إنشاء حجز جديد',()=>openEventModal()],['إضافة ضيف',()=>openGuestModal()],['فتح غرفة العمليات',()=>go('/admin/operations')],['فتح مركز الرسائل',()=>go('/admin/messages')],['فتح العملاء',()=>go('/admin/clients')],['فتح الحسابات',()=>go('/admin/accounts')],['فتح التكاملات',()=>go('/admin/integrations')],...db.bookings.map(b=>[b.eventName,()=>{setSelectedBookingId(b.id);go('/admin/workspace')}]),...db.guests.map(g=>[g.guestName,()=>{go('/admin/guests');setTimeout(()=>openGuestPage(g.id),100)}])].filter(x=>x[0].includes(q)); $('#cmdResults').innerHTML=items.slice(0,9).map((x,i)=>`<div class="cmd-item" onclick="window.__cmd[${i}]();toggleCommand(false)">${x[0]}</div>`).join(''); window.__cmd=items.map(x=>x[1]);}
function calcPrice(){
